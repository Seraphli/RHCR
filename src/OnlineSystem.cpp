...
OnlineSystem::OnlineSystem(OnlineGrid &G, MAPFSolver &solver)
...
void OnlineSystem::update_start_and_goal_locations(int num_of_new_agents) {
  // update the start states of the existing agents. Their goal locations remain
  // unchanged.
  starts.clear();
  ...
  while (count < num_of_new_agents && !available_entries.empty()) {
    ...
    paths.emplace_back(
        timestep); // insert a new path of length=timestep for the new agent
  ...
  cout << count << " new agents arrive and there are " << num_of_drives
       << " agents in total." << endl;
  ...
  for (int t = start_timestep; t <= end_timestep; t++) {
    ...
    while (path != paths.end()) {
      ...
      if (t > 0 && path->at(t - 1).timestep >= 0) {
        ...
        if (curr.location == prev.location) {
          if (G.get_rotate_degree(prev.orientation, curr.orientation) == 2) {
            cout << "A drive rotates 180 degrees from " << prev << " to "
                 << curr << endl;
            save_results();
            exit(-1);
          }
          ...
          if (prev.orientation != curr.orientation) {
            cout << "A drive rotates while moving from " << prev << " to "
                 << curr << endl;
            save_results();
            exit(-1);
      ...
      for (; other_path != paths.end(); ++other_path) {
        for (int i = max(0, t - k_robust); i <= min(t + k_robust, end_timestep);
             i++) {
          if ((int)other_path->size() <= i)
            break;
          if (other_path->at(i).location == curr.location) {
            cout << "Two drives have a conflict at location " << curr.location
                 << " at timestep " << i << endl;
      ...
      if ((int)path->size() == t + 1 &&
          path->back().location == goal->at(0).first) {
        ...
        finished_paths.back().insert(finished_paths.back().end(), start,
                                     path->end());
...
void OnlineSystem::simulate(int simulation_time) {
  ...
  for (; timestep < simulation_time; timestep += simulation_window) {
    ...
    move();
    std::cout << old - paths.size()
              << " agents have reached their goal locations" << std::endl;
