...
public:
  ...
  double get_weight(int from,
                    int to) const; // fiducials from and to are neighbors
  vector<vector<double>> get_weights() const { return weights; }
  int get_rotate_degree(int dir1,
                        int dir2) const; // return 0 if it is 0; return 1 if it
                                         // is +-90; return 2 if it is 180
  ...
  vector<double>
  compute_heuristics(int root_location); // compute distances from all lacations
  ...
  void ensure_heuristic(int loc) {
    if (heuristics.find(loc) == heuristics.end()) {
